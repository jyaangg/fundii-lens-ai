      ...
      <Card className="shadow-soft">
        ...
        <CardContent>
          <div className="space-y-4">
            {borrowerAttention.map((borrower, index) => (
              <div key={index} className="flex items-center justify-between p-4 rounded-lg border bg-muted/30">
                <div className="flex-1">
                  <h4 className="font-medium">{borrower.borrower}</h4>
                  <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                    <span>Past Due: {borrower.pastDue} days</span>
                    <span>Missing: {borrower.missing} documents</span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <div className="text-sm font-medium">Risk Score</div>
                    <div className="text-xl font-bold">{borrower.riskScore}</div>
                  </div>
                  {getRiskBadge(borrower.riskScore)}
                </div>
              </div>
            ))}
          </div>
